...
function routeDetails(){ return document.querySelector('#routeEditorToolbar'); }
...
function updateOverlayInteraction(){
  const svg=floorMap();
  if(!svg)return;
  svg.classList.toggle('vre-draw-mode', active && mode==='draw');
  svg.setAttribute('aria-label', mode==='draw' ? '새 경로 그리기 모드' : '경로 선택 및 이동 모드');
}
...
function injectStyles(){
  ...
  style.textContent=`
    #routeEditorToolbar{display:flex!important;flex-direction:column!important;flex-wrap:nowrap!important;align-items:stretch!important;gap:7px!important;overflow:visible!important;scrollbar-width:thin}
    #routeEditorToolbar[hidden]{display:none!important}
    #routeEditorToolbar>*{flex:0 0 auto;width:100%}
    #routeEditorToolbar button{min-width:0!important;width:100%;padding-left:10px!important;padding-right:10px!important;white-space:nowrap;text-align:left}
    #routeEditorToolbar .vre-spacer{display:none}
    #routeEditorToolbar button.vre-toggle.active,
    #routeEditorToolbar button[data-tool].active{color:#fff;border-color:#1d4ed8;background:#1d4ed8}
    #routeEditorToolbar .vre-grid-control{display:grid;grid-template-columns:1fr 64px 24px;align-items:center;gap:5px;flex:0 0 auto;width:100%}
    #routeEditorToolbar .vre-grid-size{width:48px!important;min-width:48px;height:32px;padding:4px 5px!important;margin:0!important;border:1px solid #c7d0da;border-radius:8px;background:#fff;text-align:center;font-size:13px;font-weight:700}
    #routeEditorToolbar .vre-grid-unit{font-size:12px;color:#536174;font-weight:700;white-space:nowrap}
    #floorMap.vre-active{cursor:default!important;outline:none!important;outline-offset:0!important;pointer-events:none}
    #floorMap.vre-active.vre-draw-mode{cursor:crosshair!important;pointer-events:auto}
    #vectorRouteSvgLayer{pointer-events:none}
    #vectorRouteSvgLayer .vre-hit{pointer-events:stroke}
    #vectorRouteSvgLayer .vre-vertex{pointer-events:all}
    ...
    #vectorRouteSvgLayer .vre-segment.selected{stroke:#facc15;stroke-width:4.25}
    #vectorRouteSvgLayer .vre-segment.closed{stroke:#dc2626;stroke-dasharray:7 5;opacity:.95}
    #vectorRouteSvgLayer .vre-segment.closed.selected{stroke:#facc15;stroke-dasharray:7 5;stroke-width:4.25}
    #vectorRouteSvgLayer .vre-vertex{fill:#fff;stroke:#1d4ed8;stroke-width:2;cursor:move;vector-effect:non-scaling-stroke}
    #vectorRouteSvgLayer .vre-vertex.selected{fill:#fef08a;stroke:#eab308;stroke-width:3}
...
function ensureToolbar(){
  ...
  drawButton.title='두 지점을 한 쌍으로 선택해 독립된 경로를 만듭니다. 1·2 클릭은 1-2, 3·4 클릭은 3-4로 연결됩니다.';
...
function setMode(next){ mode=next;drawStart=null;selected=null;clearAlignmentGuides();updateOverlayInteraction();toolbar?.querySelectorAll('[data-tool]').forEach(b=>b.classList.toggle('active',b.dataset.tool===next));render();setStatus(next==='draw'?'경로 추가 활성화: 시작점과 끝점을 두 번 클릭하면 경로 1개가 만들어집니다. 다음 두 번의 클릭은 별도의 새 경로가 됩니다.':'선택·이동: 빈 지도는 MapLibre가 이동하고, 경로 선·꼭지점만 편집됩니다.'); }
...
function handlePairedDrawVertex(vertex, firstMessage='시작점을 선택했습니다. 끝점을 한 번 더 클릭하세요.', completeMessage='두 지점을 연결해 새 경로를 만들었습니다. 다음 클릭은 새 경로의 시작점입니다.'){
  if(!vertex) return;

  if(!drawStart){
    drawStart=vertex.id;
    selected={type:'vertex',id:vertex.id};
    commit(firstMessage);
    return;
  }

  const start=vertexById(drawStart);
  if(!start){
    drawStart=vertex.id;
    selected={type:'vertex',id:vertex.id};
    commit(firstMessage);
  ...
  if(start.id===vertex.id){
    selected={type:'vertex',id:vertex.id};
    render();
    setStatus('시작점과 같은 점입니다. 다른 끝점을 클릭하세요.');
    return;
  }

  const added=addSegment(start,vertex);
  drawStart=null;
  selected=added?{type:'segment',id:added.id}:{type:'vertex',id:vertex.id};
  processIntersections();
  commit(added?completeMessage:'두 지점 사이의 경로가 이미 존재합니다. 다음 클릭은 새 경로의 시작점입니다.');
}
...
function onPointerDown(event){
  ...
  if(vertexEl){event.preventDefault();event.stopPropagation();const id=vertexEl.dataset.vertexId;if(mode==='draw'){handlePairedDrawVertex(vertexById(id));}else{selected={type:'vertex',id};drag={id,start:clone(vertexById(id)),moved:false};render();}return;}
  if(segmentEl){event.preventDefault();event.stopPropagation();const id=segmentEl.dataset.segmentId;if(mode==='select'){selected={type:'segment',id};render();setStatus(segmentById(id)?.closed===true?'폐쇄된 경로를 선택했습니다. 경로 폐쇄·재개 버튼으로 복구할 수 있습니다.':'경로를 선택했습니다. 삭제하거나 폐쇄할 수 있습니다.');}else{const v=vertexOnSegment(segmentById(id),p);if(v){handlePairedDrawVertex(v,'기존 선 위의 시작점을 선택했습니다. 끝점을 한 번 더 클릭하세요.','기존 선의 접점과 끝점을 연결해 새 경로를 만들었습니다. 다음 클릭은 새 경로의 시작점입니다.');}}return;}
  if(mode==='draw'){event.preventDefault();event.stopPropagation();const q=snapPoint(p);let v=findVertexNear(q,null,HIT_DISTANCE);if(!v)v=addVertex(q);handlePairedDrawVertex(v);}else{selected=null;render();}
}
...
function refreshProjection(){
  ...
  const keepSelected=selected?{...selected}:null;
  const keepDrawStart=drawStart;
  graph=normalizeInput(bridge()?.exportCurrentGraph?.()||{vertices:[],segments:[]});
  if(keepSelected){
    if(keepSelected.type==='vertex'&&vertexById(keepSelected.id))selected=keepSelected;
    else if(keepSelected.type==='segment'&&segmentById(keepSelected.id))selected=keepSelected;
    else selected=null;
  }
  drawStart=keepDrawStart&&vertexById(keepDrawStart)?keepDrawStart:null;
  render();
}
function closeEditor(options={}){
  ...
  group?.remove();
  group=null;
  floorMap()?.classList.remove('vre-active','vre-draw-mode');
...
window.JointRouteEditor=Object.freeze({open:openEditor,close:closeEditor,refreshProjection,suspendProjection:()=>{},resumeProjection:refreshProjection,isOpen:()=>active});
